...
printf "\E[34m                               |_|                            \n"
printf "                                                                    \n"
printf "\e[34m          By UnknowUser50 || version 1.2.0 || 2020            \n\e[0m"
echo "                                                                       "
echo "                                                                       "
echo "                                                                       "
...
if [ $choix3 == $nikto ]; then
	...
	read target
	echo -e -n "\e[36m [+] Do you want to add cookies options ? (1/2) : "
	read resp_cook
	if [[ $resp_cook == $Yes ]]; then
		echo -n -e "\e[36m [+] Enter cookies here : \e[0m"
		read cookies
		sleep 2
		printf "\e[36m [+] SQL injection on \e[33m$target\e[36m with cookie : \e[33m$cookies\e[36m !\n"
		sleep 1
		sqlmap -u $target --cookie="$cookie" --dbs
		sleep 1
		printf "\e[36m [+] Done ! \e[0m"
	elif [[ $resp_cook == $no ]]; then
		printf "\e[36m [+] SQL injection without cookie on : \e[33m$target\e[36m\n"
		...
		printf "\e[31m [+] You have to fill in the parameters !\e[0m\n"
		sleep 1
		mainmenu
	fi	
